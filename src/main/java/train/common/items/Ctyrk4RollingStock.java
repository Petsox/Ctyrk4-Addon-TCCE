...
import train.common.Debug;
import train.common.library.Ctyrk4EnumTrains;

import javax.annotation.Nullable;
import java.util.List;

public class Ctyrk4RollingStock extends ItemMinecart implements IMinecart, IMinecartItem {
	...
	public Ctyrk4RollingStock(String iconName) {
		super(1);
		this.iconName = iconName;
		maxStackSize = 1;
		trainName = this.getUnlocalizedName();
		if(!ConfigHandler.SPLIT_CREATIVE) {
			setCreativeTab(Ctyrk4_Addon_TCCE.tc4Tab);
		} else {
			setCreativeTab(Ctyrk4_Addon_TCCE.tc4TrainTab);
		}
	}
	...
	public EntityMinecart placeCart(EntityPlayer player, ItemStack itemstack, World world, int i, int j, int k) {
		...
		if (rollingStock != null) {
			if (!world.isRemote) {
				...
				if ((rollingStock instanceof train.common.api.SteamTrain && !ConfigHandler.ENABLE_STEAM) || (rollingStock instanceof train.common.api.ElectricTrain && !ConfigHandler.ENABLE_ELECTRIC) || (rollingStock instanceof train.common.api.DieselTrain && !ConfigHandler.ENABLE_DIESEL) || (rollingStock instanceof train.common.api.Tender && !ConfigHandler.ENABLE_TENDER)) {
					if (player != null)
				...
				if (dir == 0) {
					...
					if (rollingStock instanceof train.common.api.Locomotive || rollingStock instanceof train.common.api.ISecondBogie) {
						if (Debug.isDebugEnabled()){
				...
				if (dir == 7) {
					...
					rollingStock.trainType = ((Ctyrk4RollingStock) itemstack.getItem()).getTrainType();
					...
					if (player != null)
						rollingStock.setInformation(((Ctyrk4RollingStock) itemstack.getItem()).getTrainType(), player.getDisplayName(), trainCreator, (itemstack.getItem()).getItemStackDisplayName(itemstack), uniID);
					if (player == null)
						rollingStock.setInformation(((Ctyrk4RollingStock) itemstack.getItem()).getTrainType(), "", trainCreator, (itemstack.getItem()).getItemStackDisplayName(itemstack), uniID);
	...
	public void registerIcons(IIconRegister iconRegister) {
		this.itemIcon = iconRegister.registerIcon(Ctyrk4AddonInfo.modID.toLowerCase() + ":trains/" + this.iconName);
	}
}
