...
public class GuiLoco2 extends GuiContainer {
	...
	private String texture = Ctyrk4AddonInfo.guiPrefix + "customButton.png";
	...
	private train.common.api.Locomotive loco;

	public GuiLoco2(InventoryPlayer inventoryplayer, Entity entityminecart) {
		super(new InventoryLoco(inventoryplayer, (train.common.api.Locomotive) entityminecart));
		loco = (train.common.api.Locomotive) entityminecart;
	}
	...
	public void initGui() {
		...
		if (!loco.getParkingBrakeFromPacket()) {
			if (loco instanceof train.common.api.SteamTrain) {
				textureX = 41;
				textureY = 13;
				textureSizeX = 40;
				textureSizeY = 13;
			...
			buttonPosX = 43;
			buttonPosY = -13;
			buttonList.add(new train.client.gui.GuiCustomButton(2, ((width - xSize) / 2) + buttonPosX - 12, ((height - ySize) / 2) + buttonPosY, textureSizeX, textureSizeY, "", texture, textureX, textureY));//Brake: Off
		}
		else {
			if (loco instanceof train.common.api.SteamTrain) {
				textureX = 0;
	...
	protected void actionPerformed(GuiButton guibutton) {
		if (guibutton.id == 2) {
			if ((!loco.parkingBrake) && loco.getSpeed() < 10) {
				Ctyrk4_Addon_TCCE.brakeChannel.sendToServer(new train.common.core.network.PacketParkingBrake(true, loco.getEntityId()));
				...
				Ctyrk4_Addon_TCCE.brakeChannel.sendToServer(new train.common.core.network.PacketParkingBrake(false, loco.getEntityId()));
		...
		if (guibutton.id == 3) {
			if (loco.riddenByEntity instanceof EntityPlayer && ((EntityPlayer) loco.riddenByEntity).getDisplayName().equals(loco.getTrainOwner())) {
				if ((!loco.getTrainLockedFromPacket())) {
					Ctyrk4_Addon_TCCE.lockChannel.sendToServer(new train.common.core.network.PacketSetTrainLockedToClient(true, loco.getEntityId()));
					loco.locked = true;
					guibutton.displayString = "Locked";
					this.initGui();
				}
				else {
					Ctyrk4_Addon_TCCE.lockChannel.sendToServer(new PacketSetTrainLockedToClient(false, loco.getEntityId()));
					loco.locked = false;
		...
		if (guibutton.id == 4) {
			if (loco.isLocoTurnedOn()) {
				if(loco.getSpeed() <= 1){
					Ctyrk4_Addon_TCCE.ignitionChannel.sendToServer(new train.common.core.network.PacketSetLocoTurnedOn(false));
					 ...
					 */
					Ctyrk4_Addon_TCCE.brakeChannel.sendToServer(new PacketParkingBrake(true, loco.getEntityId()));
					loco.parkingBrake = true;
					loco.isBraking = true;
					this.initGui();
				}else{
				...
				Ctyrk4_Addon_TCCE.ignitionChannel.sendToServer(new PacketSetLocoTurnedOn(true));
	...
	public void drawScreen(int mouseX, int mouseY, float par3){
		...
		if(loco instanceof train.common.api.SteamTrain){
			...
			if (mouseX>j+143 && mouseX<j+161 && mouseY>k+18 && mouseY<k+68){
				drawHoveringText(Collections.singletonList("Water: " + (((train.common.api.SteamTrain) loco).getWater()) + "mb / " + (((train.common.api.SteamTrain) loco).getCartTankCapacity()) +"mb"),
						mouseX, mouseY, fontRendererObj);
			}
		} else 	if(loco instanceof train.common.api.DieselTrain){
			int j = (width - xSize) / 2;
			int k = (height - ySize) / 2;
			if (mouseX>j+143 && mouseX<j+161 && mouseY>k+18 && mouseY<k+68){
				if (((train.common.api.DieselTrain) loco).getDiesel()!=0){
					drawHoveringText(Collections.singletonList(StatCollector.translateToLocal("fluid.tc:"+((train.common.api.DieselTrain) loco).getLiquidName()) + " " +
									((train.common.api.DieselTrain) loco).getDiesel() +"mb / " + (((train.common.api.DieselTrain) loco).getCartTankCapacity()) + "mb"),
							mouseX, mouseY, fontRendererObj);
				}else {
					drawHoveringText(Collections.singletonList("Fuel: " +
									"0mb / " + (((train.common.api.DieselTrain) loco).getCartTankCapacity()) + "mb"),
							mouseX, mouseY, fontRendererObj);
				}
			}
		}
	...
	protected void drawGuiContainerBackgroundLayer(float f, int t, int g) {
		String i = Ctyrk4AddonInfo.guiPrefix + "gui_loco.png";

		if (loco instanceof train.common.api.ElectricTrain) {
			i = Ctyrk4AddonInfo.guiPrefix + "gui_tram.png";
		}
		if (loco instanceof train.common.api.SteamTrain) {
			i = Ctyrk4AddonInfo.guiPrefix + "gui_loco_steam.png";
		}
		if (loco instanceof train.common.api.DieselTrain) {
			i = Ctyrk4AddonInfo.guiPrefix + "gui_loco_diesel.png";
		}

		GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
		mc.renderEngine.bindTexture(new ResourceLocation(Ctyrk4AddonInfo.resourceLocation, i));
		int j = (width - xSize) / 2;
		int k = (height - ySize) / 2;
		drawTexturedModalRect(j, k, 0, 0, xSize, ySize);

		if (loco instanceof train.common.api.SteamTrain) {
			int load = (((train.common.api.SteamTrain) loco).getWater());
			int lo = Math.abs(((load * 50) / (((train.common.api.SteamTrain) loco).getCartTankCapacity())));

			if (((train.common.api.SteamTrain) loco).getLiquidItemID() == train.common.api.LiquidManager.WATER_FILTER.getFluidID()) {
			...
			int load = (((train.common.api.DieselTrain) loco).getDiesel());
