...
public class HUDloco extends GuiScreen {
	...
	public void onGameRender(RenderGameOverlayEvent.Text event){
		if (game != null && game.thePlayer != null && game.thePlayer.ridingEntity != null && game.thePlayer.ridingEntity instanceof train.common.api.Locomotive && Minecraft.isGuiEnabled() && game.currentScreen == null) {
			renderSkillHUD(event, (train.common.api.Locomotive) game.thePlayer.ridingEntity);
		} else {
	...
	public void renderSkillHUD(RenderGameOverlayEvent event, train.common.api.Locomotive rcCar) {
		 ...
		 */
		if (rcCar instanceof train.common.api.SteamTrain) {
	...
	private void renderBG(train.common.api.Locomotive rcCar) {
		GL11.glEnable(3042);
		GL11.glEnable(32826);
		this.zLevel = -90.0F;
		if (rcCar instanceof train.common.api.SteamTrain) {
			game.renderEngine.bindTexture(new ResourceLocation(Ctyrk4AddonInfo.resourceLocation, Ctyrk4AddonInfo.guiPrefix + "loco_hud_steam.png"));
		}
		else {
			game.renderEngine.bindTexture(new ResourceLocation(Ctyrk4AddonInfo.resourceLocation, Ctyrk4AddonInfo.guiPrefix + "locohud.png"));
		}
	...
	private void renderText(train.common.api.Locomotive loco) {
	...
	private void renderFuelBar(train.common.api.Locomotive loco) {
		GL11.glEnable(3042 /* GL_BLEND */);
		GL11.glEnable(32826);
		int l;
		/**
		...
		if (loco instanceof train.common.api.DieselTrain) {
			l = ((((train.common.api.DieselTrain) loco).getDiesel()));
	...
	private void renderWaterBar(train.common.api.Locomotive loco) {
		int l = ((train.common.api.SteamTrain) loco).getWater();
		int l_Scaled = Math.abs((l * 49) / ((train.common.api.SteamTrain) loco).getCartTankCapacity());
		if (l_Scaled > 49) {
	...
	private void renderSpeedometer(train.common.api.Locomotive loco) {
		double speed = loco.getSpeed();
		GL11.glEnable(3042);
		GL11.glEnable(32826);
