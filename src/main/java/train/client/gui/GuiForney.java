...
public class GuiForney extends GuiContainer {
	...
	public GuiForney(InventoryPlayer inventoryplayer, Entity entityminecart) {
		super(new InventoryForney(inventoryplayer, (train.common.api.Locomotive) entityminecart));
	...
	public void initGui() {
		...
		if (!loco.parkingBrake) {
			...
			buttonPosX = 33;
			buttonPosY = -13;
			buttonList.add(new train.client.gui.GuiCustomButton(2, ((width - xSize) / 2) + buttonPosX, ((height - ySize) / 2) + buttonPosY, textureSizeX, textureSizeY, "", texture, textureX, textureY));// Brake: Off
		}
	...
	protected void actionPerformed(GuiButton guibutton) {
		if (guibutton.id == 2) {
			if ((!loco.getParkingBrakeFromPacket()) && loco.getSpeed() < 10) {
				...
				Ctyrk4_Addon_TCCE.brakeChannel.sendToServer(new PacketParkingBrake(false, loco.getEntityId()));
	...
	protected void drawGuiContainerBackgroundLayer(float f, int t, int g) {
		...
		mc.renderEngine.bindTexture(new ResourceLocation(Ctyrk4AddonInfo.resourceLocation, Ctyrk4AddonInfo.guiPrefix + "gui_forney.png"));
		int j = (width - xSize) / 2;
		int k = (height - ySize) / 2;
