...
public class GuiLiquid extends GuiContainer {
	...
	private train.common.api.LiquidTank liquid;
	private EntityPlayer player;
	
	public GuiLiquid(EntityPlayer player, InventoryPlayer inventoryplayer, Entity entityminecart) {
		super(new InventoryLiquid(inventoryplayer, (train.common.api.LiquidTank) entityminecart));
					...
					if (lis3 != null && lis3.size() > 0) {
						for (Object entity : lis3) {
							if (entity instanceof EntityPlayer) {
								Ctyrk4_Addon_TCCE.lockChannel
										.sendToServer(new train.common.core.network.PacketSetTrainLockedToClient(true, liquid.getEntityId()));
							}
						}
					}
					liquid.locked = true;
	...
	protected void drawGuiContainerBackgroundLayer(float f, int t, int g) {
		...
		mc.renderEngine.bindTexture(new ResourceLocation(Ctyrk4AddonInfo.resourceLocation, Ctyrk4AddonInfo.guiPrefix + "gui_liquid.png"));
		int j = (width - xSize) / 2;
		int k = (height - ySize) / 2;
		...
		if (theLiquid != null) {
			...
			mc.renderEngine.bindTexture(train.client.core.helpers.FluidRenderHelper.getFluidSheet(theLiquid));
			...
			for (int col = 0; col < 66 / 16; col++) {
				for (int row = 0; row <= (l) / 16; row++) {
					...
					drawTexturedModelRectFromIcon(j + 58 + col * 16, k + 52 + -row * 16, train.client.core.helpers.FluidRenderHelper.getFluidTexture(theLiquid,false), 16, 16);
				}
			}
