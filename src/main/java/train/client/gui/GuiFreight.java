...
public class GuiFreight extends GuiContainer {
	...
	private train.common.api.Freight freight;
	...
	public GuiFreight(EntityPlayer player, InventoryPlayer inventoryplayer, Entity entityminecart) {
		super(new InventoryFreight(inventoryplayer, (train.common.api.Freight) entityminecart));
			...
			if (player != null && player.getCommandSenderName().toLowerCase().equals(freight.getTrainOwner().toLowerCase())) {
				if ((!freight.getTrainLockedFromPacket())) {
					...
					if (lis3 != null && lis3.size() > 0) {
						for (Object entity : lis3) {
							if (entity instanceof EntityPlayer) {
								Ctyrk4_Addon_TCCE.lockChannel.sendToServer(new train.common.core.network.PacketSetTrainLockedToClient(true, freight.getEntityId()));
							}
						}
					}
					...
					if (lis3 != null && lis3.size() > 0) {
						for (Object entity : lis3) {
							if (entity instanceof EntityPlayer) {
								Ctyrk4_Addon_TCCE.lockChannel.sendToServer(new PacketSetTrainLockedToClient(false, freight.getEntityId()));
							}
						}
					}
	...
	protected void drawGuiContainerBackgroundLayer(float f, int t, int g) {
		...
		mc.renderEngine.bindTexture(new ResourceLocation(Ctyrk4AddonInfo.resourceLocation, Ctyrk4AddonInfo.guiPrefix + "container.png"));
		int j = (width - xSize) / 2;
		int k = (height - ySize) / 2;
