...
public class ClientTickHandler {
	...
	private void tickStart(TickEvent event) {
		if (mc.theWorld == null) { // fixes streaming after exiting a world
			for (MP3Player player : Ctyrk4_Addon_TCCE.proxy.playerList) if (player != null) player.stop();
			Ctyrk4_Addon_TCCE.proxy.playerList.clear();
		}
		...
		if(!isHidden && Loader.isModLoaded("NotEnoughItems")) {
			if(mc.theWorld != null && mc.theWorld.playerEntities != null) {
				Ctyrk4_Addon_TCCE.proxy.doNEICheck(new ItemStack(Block.getBlockFromName(Ctyrk4AddonInfo.modID + ":tcRail")));
				Ctyrk4_Addon_TCCE.proxy.doNEICheck(new ItemStack(Block.getBlockFromName(Ctyrk4AddonInfo.modID + ":tcRailGag")));
				isHidden = true;
			}
		}
	}
